...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (themeToggle) {
    ...
    themeToggle.addEventListener('click', () => {
      themeToggle.classList.add('is-animating'); // Animación de rotación
  ...
  if (preloader) {
    // Remover preloader en DOMContentLoaded con un pequeño delay
    setTimeout(() => {
  ...
  document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', function (e) {
      const href = this.getAttribute('href').split('#').pop(); // Maneja enlaces tipo index.html#proceso
      if (href === '#') return;
  ...
  if (msgArea && charCtr) {
    msgArea.addEventListener('input', () => {
      ...
      const max = parseInt(msgArea.getAttribute('maxlength'), 10) || 500;
      charCtr.textContent = `${len} / ${max}`;
      charCtr.classList.toggle('near-limit', len >= max * 0.8 && len < max);
      charCtr.classList.toggle('at-limit',   len >= max);
    });
  }
});
